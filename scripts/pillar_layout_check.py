...
sampler is supposed to guarantee:
  ...
  5. corridor width               min over pillar pairs of (||dxy|| - 2*pillar_radius)
                                  and, with --min-corridor, against the A3 bound
                                  2*(r_o + drone_radius + inflation) + 0.25
  6. connectivity (--connectivity) 2-D grid BFS start -> goal over the discs that
                                  intersect the flight band. Coarse (2-D slice, cell
                                  size --cell): it proves "a gap exists in xy", not
                                  that the drone can fly it. Enough to catch layouts
                                  where the goal is walled off, which is what A3 must
                                  not ship.
...
def main():
    ...
    ap.add_argument("--min-corridor", action="store_true",
                    help="also apply the A3 corridor bound 2*(r_o+dr+inflation)+0.25")
    ...
    M = nP * L + nF
    print(f"[check] {a.profile}: n_pillars={nP} layers={L} n_free={nF} -> M={M} "
          f"(max_slots={oc.get('max_slots')})")
    ...
    if a.min_corridor:
        print(f"[check] A3 corridor floor = 2*(r_o+dr+infl)+0.25 = {corridor_floor:.4f} m")
    ...
    for s in range(a.layouts):
        ...
        if a.min_corridor and mc_corr < corridor_floor - 1e-6:
            probs.append(f"corridor {mc_corr:.4f} < {corridor_floor:.4f}")
        ...
        if a.connectivity:
            ...
            for i in range(M):
                ...
                blocked |= d < (r_s[0, i] + float(mgr.danger_radius) * 0.0)
        ...
        if probs:
            ...
            print(f"  layout {s}: ok  M={int(n_act[0])} init_clr={mc_init:.3f} "
